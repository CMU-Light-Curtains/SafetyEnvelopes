...
var KittiViewer = function (pointCloud, lcCurtain, sbCloud, logger, imagePanel, imageCanvas, scorePanel, scoreCanvas, bevHmapManager, safetyEnvelope) {
    ...
    this.lcVizMode = "Full Curtain";
    this.isectThresh = 0.5;
...
KittiViewer.prototype = {
    ...
    connectSceneCloudStream: function() {
        ...
        sceneCloudSource.onmessage = function(e) {
            ...
            if ("se_pc_str" in data) {
                const se_points_buf = str2buffer(atob(data["se_pc_str"]))
                let se_points;
                if (self.enableInt16) {
                    const se_points_int = new Int16Array(se_points_buf);
                    se_points = new Float32Array(se_points_int.length);
                    for (let i = 0; i < se_points_int.length; i++)
                        se_points[i] = se_points_int[i] / self.int16Factor;
                } else {
                    se_points = new Float32Array(points_buf);
                }
                self.safetyEnvelope.update(se_points);
            }
            else {
                self.safetyEnvelope.invisible();
            }
        };
    ...
    connectLcCurtainStream: function() {
        let self = this;
        var lcCloudSource = new EventSource(self.addhttp(self.backend) + "/api/stream_lc_curtain");
        lcCloudSource.onmessage = function(e) {
            ...
            let curtainBoundary, isectPoints;
            if (self.enableInt16){
                // boundary
                let intArray = new Int16Array(boundaryBuf);
                curtainBoundary = new Float32Array(intArray.length);
                for (let i = 0; i < intArray.length; i++)
                    curtainBoundary[i] = intArray[i] / self.int16Factor;

                // intersection points
                intArray = new Int16Array(isectPointsBuf);
                isectPoints = new Float32Array(intArray.length);
                for (let i = 0; i < intArray.length; i++)
                    isectPoints[i] = intArray[i] / self.int16Factor;
            }
            else{
                curtainBoundary = new Float32Array(boundaryBuf);
                isectPoints = new Float32Array(isectPointsBuf);
            }
            ...
            //     }
            //
            //     if (self.lcVizMode == 'Intersection' && intensity <= self.isectThresh)
            //         continue;
            //
            //     // Position
            //     self.lcCloud.geometry.attributes.position.array[ptIter * 3] = x;
            //     self.lcCloud.geometry.attributes.position.array[ptIter * 3 + 1] = y;
            //     self.lcCloud.geometry.attributes.position.array[ptIter * 3 + 2] = z;
            //
            //     // Color.
            //     self.lcCloud.geometry.attributes.color.array[3 * ptIter] = r;
            //     self.lcCloud.geometry.attributes.color.array[3 * ptIter + 1] = intensity;
            //     self.lcCloud.geometry.attributes.color.array[3 * ptIter + 2] = b;
            //
            //     ptIter += 1;
            // }
            // self.lcCloud.geometry.setDrawRange(0, Math.min(ptIter, self.maxPoints));
            // self.lcCloud.geometry.attributes.position.needsUpdate = true;
            // self.lcCloud.geometry.attributes.color.needsUpdate = true;
            // self.lcCloud.geometry.computeBoundingSphere();

            // Write the curtain's score onto score canvas.
            let score = data["score"];
            if (score != null) {
                score = score.toFixed(4);

                // set the text color
                const FULLY_RED_SCORE = 0.3;  // score when color is fully red
                let color = Math.floor(255 * score / FULLY_RED_SCORE)
                ...
                // Write the curtain's score onto score canvas.
                let ctx = self.scoreCanvas.getContext("2d");
                ctx.clearRect(0, 0, self.scoreCanvas.width, self.scoreCanvas.height);
     ...
     toggleLCVizMode: function() {
        let self = this;
        if (self.lcVizMode == 'Full Curtain') self.lcVizMode = 'Intersection';
        else self.lcVizMode = 'Full Curtain';
    },
    ...
    // },
    // connectArrowsStream: function() {
    //     let self = this;
    //     var velocitySource = new EventSource(self.addhttp(self.backend) + "/api/stream_arrows");
    //     velocitySource.onmessage = function(e) {
    //         // Add LC point cloud (visible).
    //         var arrows = JSON.parse(e.data);
    //         var tails = arrows["tails"];
    //         var heads = arrows["heads"];
    //         for (var i = 0; i < self.arrows.length; ++i) {
    //             scene.remove(self.arrows[i]);
    //         }
    //         self.arrows = makeArrows(tails, heads);
    //         for (var i = 0; i < self.arrows.length; ++i) {
    //             scene.add(self.arrows[i]);
    //         }
    //     };
    //     console.log("Connected to velocity stream.")
    // },
